...
satisfaction.ratings".Split(Environment.NewLine, StringSplitOptions.RemoveEmptyEntries);
        ...
        public void IsAuthenticWebhook_Works()
        {
            ...
            Assert.IsFalse(WebhookService.IsAuthenticWebhook(secretKey, invalidSignature, body));
            Assert.IsTrue(WebhookService.IsAuthenticWebhook(secretKey, validSignature, body));
        }
    }
}
